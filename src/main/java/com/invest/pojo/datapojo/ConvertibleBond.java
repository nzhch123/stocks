package com.invest.pojo.datapojo;

import java.util.Date;

public class ConvertibleBond {
    private String bondId;

    private String bondNm;

    private String stockId;

    private String stockNm;

    private String btype;

    private String convertPrice;

    private String convertPriceValidFrom;

    private Date convertDt;

    private Date maturityDt;

    private Date nextPutDt;

    private Date putDt;

    private String putNotes;

    private String putPrice;

    private String putIncCpnFl;

    private String putConvertPriceRatio;

    private int putCountDays;

    private int putTotalDays;

    private int putRealDays;

    private String repoDiscountRt;

    private String repoValidFrom;

    private String repoValidTo;

    private String turnoverRt;

    private String redeemPrice;

    private String redeemIncCpnFl;

    private String redeemPriceRatio;

    private int redeemCountDays;

    private int redeemTotalDays;

    private int redeemRealDays;

    private Date redeemDt;

    private String redeemFlag;

    private String origIssAmt;

    private String currIssAmt;

    private String ratingCd;

    private String issuerRatingCd;

    private String guarantor;

    private Date sscDt;

    private Date escDt;

    private String scNotes;

    private String marketCd;

    private String forceRedeem;

    private String realForceRedeemPrice;

    private String convertCd;

    private String repoCd;

    private String ration;

    private String rationCd;

    private String applyCd;

    private String onlineOfflineRatio;

    private String qflag;

    private String qflag2;

    private String rationRt;

    private String fundRt;

    private String marginFlg;

    private String pb;

    private String pbFlag;

    private String totalShares;

    private String floatShares;

    private String sqflg;

    private String sprice;

    private String svolume;

    private String sincreaseRt;

    private String qstatus;

    private String bondValue;

    private String bondValue2;

    private String volatilityRate;

    private String lastTime;

    private String convertValue;

    private String premiumRt;

    private String yearLeft;

    private String ytmRt;

    private String ytmRtTax;

    private String price;

    private String fullPrice;
    //转债的涨幅
    private String increaseRt;

    private String volume;

    private String convertPriceValid;

    private int adjScnt;

    private int adjCnt;

    private String redeemIcon;

    private String refYieldInfo;

    private String adjustTip;

    private String adjusted;

    private String optionTip;

    private String bondValue3;

    private String leftPutYear;

    private String shortMaturityDt;

    private String dblow;

    private String force_redeem_price;

    private String put_convert_price;

    private String convert_amt_ratio;

    private String convert_amt_ratio2;

    private String convertAmtRatioTips;

    private String stockNetValue;

    private String stockCd;

    private String preBondId;

    private String repoValid;

    private String convertCdTip;

    private String priceTips;

    public void setBondId(String bondId){
        this.bondId = bondId;
    }
    public String getBondId(){
        return this.bondId;
    }
    public void setBondNm(String bondNm){
        this.bondNm = bondNm;
    }
    public String getBondNm(){
        return this.bondNm;
    }
    public void setStockId(String stockId){
        this.stockId = stockId;
    }
    public String getStockId(){
        return this.stockId;
    }
    public void setStockNm(String stockNm){
        this.stockNm = stockNm;
    }
    public String getStockNm(){
        return this.stockNm;
    }
    public void setBtype(String btype){
        this.btype = btype;
    }
    public String getBtype(){
        return this.btype;
    }
    public void setConvertPrice(String convertPrice){
        this.convertPrice = convertPrice;
    }
    public String getConvertPrice(){
        return this.convertPrice;
    }
    public void setConvertPriceValidFrom(String convertPriceValidFrom){
        this.convertPriceValidFrom = convertPriceValidFrom;
    }
    public String getConvertPriceValidFrom(){
        return this.convertPriceValidFrom;
    }
    public void setConvertDt(Date convertDt){
        this.convertDt = convertDt;
    }
    public Date getConvertDt(){
        return this.convertDt;
    }
    public void setMaturityDt(Date maturityDt){
        this.maturityDt = maturityDt;
    }
    public Date getMaturityDt(){
        return this.maturityDt;
    }
    public void setNextPutDt(Date nextPutDt){
        this.nextPutDt = nextPutDt;
    }
    public Date getNextPutDt(){
        return this.nextPutDt;
    }
    public void setPutDt(Date putDt){
        this.putDt = putDt;
    }
    public Date getPutDt(){
        return this.putDt;
    }
    public void setPutNotes(String putNotes){
        this.putNotes = putNotes;
    }
    public String getPutNotes(){
        return this.putNotes;
    }
    public void setPutPrice(String putPrice){
        this.putPrice = putPrice;
    }
    public String getPutPrice(){
        return this.putPrice;
    }
    public void setPutIncCpnFl(String putIncCpnFl){
        this.putIncCpnFl = putIncCpnFl;
    }
    public String getPutIncCpnFl(){
        return this.putIncCpnFl;
    }
    public void setPutConvertPriceRatio(String putConvertPriceRatio){
        this.putConvertPriceRatio = putConvertPriceRatio;
    }
    public String getPutConvertPriceRatio(){
        return this.putConvertPriceRatio;
    }
    public void setPutCountDays(int putCountDays){
        this.putCountDays = putCountDays;
    }
    public int getPutCountDays(){
        return this.putCountDays;
    }
    public void setPutTotalDays(int putTotalDays){
        this.putTotalDays = putTotalDays;
    }
    public int getPutTotalDays(){
        return this.putTotalDays;
    }
    public void setPutRealDays(int putRealDays){
        this.putRealDays = putRealDays;
    }
    public int getPutRealDays(){
        return this.putRealDays;
    }
    public void setRepoDiscountRt(String repoDiscountRt){
        this.repoDiscountRt = repoDiscountRt;
    }
    public String getRepoDiscountRt(){
        return this.repoDiscountRt;
    }
    public void setRepoValidFrom(String repoValidFrom){
        this.repoValidFrom = repoValidFrom;
    }
    public String getRepoValidFrom(){
        return this.repoValidFrom;
    }
    public void setRepoValidTo(String repoValidTo){
        this.repoValidTo = repoValidTo;
    }
    public String getRepoValidTo(){
        return this.repoValidTo;
    }
    public void setTurnoverRt(String turnoverRt){
        this.turnoverRt = turnoverRt;
    }
    public String getTurnoverRt(){
        return this.turnoverRt;
    }
    public void setRedeemPrice(String redeemPrice){
        this.redeemPrice = redeemPrice;
    }
    public String getRedeemPrice(){
        return this.redeemPrice;
    }
    public void setRedeemIncCpnFl(String redeemIncCpnFl){
        this.redeemIncCpnFl = redeemIncCpnFl;
    }
    public String getRedeemIncCpnFl(){
        return this.redeemIncCpnFl;
    }
    public void setRedeemPriceRatio(String redeemPriceRatio){
        this.redeemPriceRatio = redeemPriceRatio;
    }
    public String getRedeemPriceRatio(){
        return this.redeemPriceRatio;
    }
    public void setRedeemCountDays(int redeemCountDays){
        this.redeemCountDays = redeemCountDays;
    }
    public int getRedeemCountDays(){
        return this.redeemCountDays;
    }
    public void setRedeemTotalDays(int redeemTotalDays){
        this.redeemTotalDays = redeemTotalDays;
    }
    public int getRedeemTotalDays(){
        return this.redeemTotalDays;
    }
    public void setRedeemRealDays(int redeemRealDays){
        this.redeemRealDays = redeemRealDays;
    }
    public int getRedeemRealDays(){
        return this.redeemRealDays;
    }
    public void setRedeemDt(Date redeemDt){
        this.redeemDt = redeemDt;
    }
    public Date getRedeemDt(){
        return this.redeemDt;
    }
    public void setRedeemFlag(String redeemFlag){
        this.redeemFlag = redeemFlag;
    }
    public String getRedeemFlag(){
        return this.redeemFlag;
    }
    public void setOrigIssAmt(String origIssAmt){
        this.origIssAmt = origIssAmt;
    }
    public String getOrigIssAmt(){
        return this.origIssAmt;
    }
    public void setCurrIssAmt(String currIssAmt){
        this.currIssAmt = currIssAmt;
    }
    public String getCurrIssAmt(){
        return this.currIssAmt;
    }
    public void setRatingCd(String ratingCd){
        this.ratingCd = ratingCd;
    }
    public String getRatingCd(){
        return this.ratingCd;
    }
    public void setIssuerRatingCd(String issuerRatingCd){
        this.issuerRatingCd = issuerRatingCd;
    }
    public String getIssuerRatingCd(){
        return this.issuerRatingCd;
    }
    public void setGuarantor(String guarantor){
        this.guarantor = guarantor;
    }
    public String getGuarantor(){
        return this.guarantor;
    }
    public void setSscDt(Date sscDt){
        this.sscDt = sscDt;
    }
    public Date getSscDt(){
        return this.sscDt;
    }
    public void setEscDt(Date escDt){
        this.escDt = escDt;
    }
    public Date getEscDt(){
        return this.escDt;
    }
    public void setScNotes(String scNotes){
        this.scNotes = scNotes;
    }
    public String getScNotes(){
        return this.scNotes;
    }
    public void setMarketCd(String marketCd){
        this.marketCd = marketCd;
    }
    public String getMarketCd(){
        return this.marketCd;
    }
    public void setForceRedeem(String forceRedeem){
        this.forceRedeem = forceRedeem;
    }
    public String getForceRedeem(){
        return this.forceRedeem;
    }
    public void setRealForceRedeemPrice(String realForceRedeemPrice){
        this.realForceRedeemPrice = realForceRedeemPrice;
    }
    public String getRealForceRedeemPrice(){
        return this.realForceRedeemPrice;
    }
    public void setConvertCd(String convertCd){
        this.convertCd = convertCd;
    }
    public String getConvertCd(){
        return this.convertCd;
    }
    public void setRepoCd(String repoCd){
        this.repoCd = repoCd;
    }
    public String getRepoCd(){
        return this.repoCd;
    }
    public void setRation(String ration){
        this.ration = ration;
    }
    public String getRation(){
        return this.ration;
    }
    public void setRationCd(String rationCd){
        this.rationCd = rationCd;
    }
    public String getRationCd(){
        return this.rationCd;
    }
    public void setApplyCd(String applyCd){
        this.applyCd = applyCd;
    }
    public String getApplyCd(){
        return this.applyCd;
    }
    public void setOnlineOfflineRatio(String onlineOfflineRatio){
        this.onlineOfflineRatio = onlineOfflineRatio;
    }
    public String getOnlineOfflineRatio(){
        return this.onlineOfflineRatio;
    }
    public void setQflag(String qflag){
        this.qflag = qflag;
    }
    public String getQflag(){
        return this.qflag;
    }
    public void setQflag2(String qflag2){
        this.qflag2 = qflag2;
    }
    public String getQflag2(){
        return this.qflag2;
    }
    public void setRationRt(String rationRt){
        this.rationRt = rationRt;
    }
    public String getRationRt(){
        return this.rationRt;
    }
    public void setFundRt(String fundRt){
        this.fundRt = fundRt;
    }
    public String getFundRt(){
        return this.fundRt;
    }
    public void setMarginFlg(String marginFlg){
        this.marginFlg = marginFlg;
    }
    public String getMarginFlg(){
        return this.marginFlg;
    }
    public void setPb(String pb){
        this.pb = pb;
    }
    public String getPb(){
        return this.pb;
    }
    public void setPbFlag(String pbFlag){
        this.pbFlag = pbFlag;
    }
    public String getPbFlag(){
        return this.pbFlag;
    }
    public void setTotalShares(String totalShares){
        this.totalShares = totalShares;
    }
    public String getTotalShares(){
        return this.totalShares;
    }
    public void setFloatShares(String floatShares){
        this.floatShares = floatShares;
    }
    public String getFloatShares(){
        return this.floatShares;
    }
    public void setSqflg(String sqflg){
        this.sqflg = sqflg;
    }
    public String getSqflg(){
        return this.sqflg;
    }
    public void setSprice(String sprice){
        this.sprice = sprice;
    }
    public String getSprice(){
        return this.sprice;
    }
    public void setSvolume(String svolume){
        this.svolume = svolume;
    }
    public String getSvolume(){
        return this.svolume;
    }
    public void setSincreaseRt(String sincreaseRt){
        this.sincreaseRt = sincreaseRt;
    }
    public String getSincreaseRt(){
        return this.sincreaseRt;
    }
    public void setQstatus(String qstatus){
        this.qstatus = qstatus;
    }
    public String getQstatus(){
        return this.qstatus;
    }
    public void setBondValue(String bondValue){
        this.bondValue = bondValue;
    }
    public String getBondValue(){
        return this.bondValue;
    }
    public void setBondValue2(String bondValue2){
        this.bondValue2 = bondValue2;
    }
    public String getBondValue2(){
        return this.bondValue2;
    }
    public void setVolatilityRate(String volatilityRate){
        this.volatilityRate = volatilityRate;
    }
    public String getVolatilityRate(){
        return this.volatilityRate;
    }
    public void setLastTime(String lastTime){
        this.lastTime = lastTime;
    }
    public String getLastTime(){
        return this.lastTime;
    }
    public void setConvertValue(String convertValue){
        this.convertValue = convertValue;
    }
    public String getConvertValue(){
        return this.convertValue;
    }
    public void setPremiumRt(String premiumRt){
        this.premiumRt = premiumRt;
    }
    public String getPremiumRt(){
        return this.premiumRt;
    }
    public void setYearLeft(String yearLeft){
        this.yearLeft = yearLeft;
    }
    public String getYearLeft(){
        return this.yearLeft;
    }
    public void setYtmRt(String ytmRt){
        this.ytmRt = ytmRt;
    }
    public String getYtmRt(){
        return this.ytmRt;
    }
    public void setYtmRtTax(String ytmRtTax){
        this.ytmRtTax = ytmRtTax;
    }
    public String getYtmRtTax(){
        return this.ytmRtTax;
    }
    public void setPrice(String price){
        this.price = price;
    }
    public String getPrice(){
        return this.price;
    }
    public void setFullPrice(String fullPrice){
        this.fullPrice = fullPrice;
    }
    public String getFullPrice(){
        return this.fullPrice;
    }
    public void setIncreaseRt(String increaseRt){
        this.increaseRt = increaseRt;
    }
    public String getIncreaseRt(){
        return this.increaseRt;
    }
    public void setVolume(String volume){
        this.volume = volume;
    }
    public String getVolume(){
        return this.volume;
    }
    public void setConvertPriceValid(String convertPriceValid){
        this.convertPriceValid = convertPriceValid;
    }
    public String getConvertPriceValid(){
        return this.convertPriceValid;
    }
    public void setAdjScnt(int adjScnt){
        this.adjScnt = adjScnt;
    }
    public int getAdjScnt(){
        return this.adjScnt;
    }
    public void setAdjCnt(int adjCnt){
        this.adjCnt = adjCnt;
    }
    public int getAdjCnt(){
        return this.adjCnt;
    }
    public void setRedeemIcon(String redeemIcon){
        this.redeemIcon = redeemIcon;
    }
    public String getRedeemIcon(){
        return this.redeemIcon;
    }
    public void setRefYieldInfo(String refYieldInfo){
        this.refYieldInfo = refYieldInfo;
    }
    public String getRefYieldInfo(){
        return this.refYieldInfo;
    }
    public void setAdjustTip(String adjustTip){
        this.adjustTip = adjustTip;
    }
    public String getAdjustTip(){
        return this.adjustTip;
    }
    public void setAdjusted(String adjusted){
        this.adjusted = adjusted;
    }
    public String getAdjusted(){
        return this.adjusted;
    }
    public void setOptionTip(String optionTip){
        this.optionTip = optionTip;
    }
    public String getOptionTip(){
        return this.optionTip;
    }
    public void setBondValue3(String bondValue3){
        this.bondValue3 = bondValue3;
    }
    public String getBondValue3(){
        return this.bondValue3;
    }
    public void setLeftPutYear(String leftPutYear){
        this.leftPutYear = leftPutYear;
    }
    public String getLeftPutYear(){
        return this.leftPutYear;
    }
    public void setShortMaturityDt(String shortMaturityDt){
        this.shortMaturityDt = shortMaturityDt;
    }
    public String getShortMaturityDt(){
        return this.shortMaturityDt;
    }
    public void setDblow(String dblow){
        this.dblow = dblow;
    }
    public String getDblow(){
        return this.dblow;
    }
    public void setForce_redeem_price(String force_redeem_price){
        this.force_redeem_price = force_redeem_price;
    }
    public String getForce_redeem_price(){
        return this.force_redeem_price;
    }
    public void setPut_convert_price(String put_convert_price){
        this.put_convert_price = put_convert_price;
    }
    public String getPut_convert_price(){
        return this.put_convert_price;
    }
    public void setConvert_amt_ratio(String convert_amt_ratio){
        this.convert_amt_ratio = convert_amt_ratio;
    }
    public String getConvert_amt_ratio(){
        return this.convert_amt_ratio;
    }
    public void setConvert_amt_ratio2(String convert_amt_ratio2){
        this.convert_amt_ratio2 = convert_amt_ratio2;
    }
    public String getConvert_amt_ratio2(){
        return this.convert_amt_ratio2;
    }
    public void setConvertAmtRatioTips(String convertAmtRatioTips){
        this.convertAmtRatioTips = convertAmtRatioTips;
    }
    public String getConvertAmtRatioTips(){
        return this.convertAmtRatioTips;
    }
    public void setStockNetValue(String stockNetValue){
        this.stockNetValue = stockNetValue;
    }
    public String getStockNetValue(){
        return this.stockNetValue;
    }
    public void setStockCd(String stockCd){
        this.stockCd = stockCd;
    }
    public String getStockCd(){
        return this.stockCd;
    }
    public void setPreBondId(String preBondId){
        this.preBondId = preBondId;
    }
    public String getPreBondId(){
        return this.preBondId;
    }
    public void setRepoValid(String repoValid){
        this.repoValid = repoValid;
    }
    public String getRepoValid(){
        return this.repoValid;
    }
    public void setConvertCdTip(String convertCdTip){
        this.convertCdTip = convertCdTip;
    }
    public String getConvertCdTip(){
        return this.convertCdTip;
    }
    public void setPriceTips(String priceTips){
        this.priceTips = priceTips;
    }
    public String getPriceTips(){
        return this.priceTips;
    }

}
